...
// Build2 - perform resolution
func (r *resolver) Build2(path string, values ...any) string {
	if len(path) == 0 {
		return "resolver error: invalid argument, path is empty"
	}
	if !strings.HasPrefix(path, "/") {
	...
	if r.override != nil {
		if uri, ok := r.OverrideUrl2(path); ok {
			return uri
		}
	}
	url2 := SchemeHttp + "://" + localAuthority + path
	return url2
}
...
// OverrideUrl2 - return the overridden URL
func (r *resolver) OverrideUrl2(key string) (string, bool) {
	if r.override == nil {
		return "", false
	}
	if v, ok := r.override.Load(key); ok {
