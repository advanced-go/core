...
func ExampleController_newController_Error() {
	...
	route = NewRoute("test", EgressTraffic, "", false, NewTimeoutConfig(true, 0, time.Millisecond*1500))
	_, errs = newController(route, t)
	fmt.Printf("test: newController() -> [errs:%v]\n", errs)

	route = NewRoute("test", EgressTraffic, "", false, NewTimeoutConfig(true, 0, -1))
	_, errs = newController(route, t)
	fmt.Printf("test: newController() -> [errs:%v]\n", errs)

	route = NewRoute("test", EgressTraffic, "", false, NewTimeoutConfig(true, 0, 10))
	_, errs = newController(route, t)
	fmt.Printf("test: newController() -> [errs:%v]\n", errs)
